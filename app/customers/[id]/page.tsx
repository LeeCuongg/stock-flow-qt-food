...
interface CustomerPrice {
  id: string; product_id: string; sale_price: number; updated_at: string
...
export default function CustomerDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [custRes, priceRes] = await Promise.all([
      ...
      supabase
        ...
        .select('id, product_id, sale_price, updated_at, products(name, unit, sku, product_categories(name))')
        .eq('customer_id', id)
    ...
    setPrices((priceRes.data as unknown as CustomerPrice[]) || [])
    // Load debt
    const { data: wh } = await supabase.from('warehouses').select('id').limit(1)
    if (wh?.[0]?.id) {
      const { data: debtData } = await supabase.rpc('get_receivable_report', { p_warehouse_id: wh[0].id })
      if (debtData) {
        const found = (debtData as { customer_id: string; total_receivable: number }[]).find(r => r.customer_id === id)
        setDebt(found ? Number(found.total_receivable) : 0)
      }
    }
    setLoading(false)
  }, [id, router])
  ...
  const openEditInfo = () => {
    if (!customer) return
    setEditForm({ name: customer.name, phone: customer.phone || '', address: customer.address || '', note: customer.note || '' })
    setEditOpen(true)
  }
  ...
  const startEditPrice = (p: CustomerPrice) => {
    setEditingPriceId(p.id)
    setEditPriceValue(Number(p.sale_price))
  }
  ...
  const savePrice = async (priceRow: CustomerPrice) => {
    if (editPriceValue < 0) { toast.error('Giá không hợp lệ'); return }
    setPriceSaving(true)
    try {
      const { error } = await supabase
        .from('customer_product_prices')
        .update({ sale_price: editPriceValue, updated_at: new Date().toISOString() })
        .eq('id', priceRow.id)
      if (error) throw error
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Thông tin và bảng giá của {customer.name}</p>
        </div>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{formatVNDate(customer.created_at)}</span>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          {prices.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              Chưa có lịch sử giá bán cho khách hàng này
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Giá bán gần nhất</TableHead>
